...
class CommandFilter:
    def __init__(self):
        ...
        self.allowed_commands = ['dir', 'echo', 'ping', 'whoami', 'sysinfo', 'netstat', 'tasklist', 'filesend', 'ipconfig']  # Whitelist
        self.blocked_commands = ['del','erase','rmdir', 'rd', 'ren', 'rename','move', 'shutdown', 'restart', 'net user', 'net localgroup', 'icacls', 'format', 'diskpart', 
    'chkdsk', 'ftp', 'telnet', 'arp', 'route', 'taskkill', 'sc stop', 'reg add', 'reg delete', 'regedit', 'start', 'powershell', 'certutil', 'mshta', 'msconfig', 'sc config']  # Blacklist
...
class Client:
        ...
        try:
            ...
            while True:
                ...
                if command.lower() == 'filesend':
                    print("[+] Recieved filesend command from the server.")
                    self.receive_file()
                             
                if command:
                    # Filter and execute the command
    ...
    def receive_file(self):
        try:
            ...
            print(f"[+] Error recieving file: {e}")
